...
layout(location = 4) in float v_rounded_exponent;
layout(location = 5) in vec4 v_border_color;
layout(location = 6) in float v_border_width;
...
vec4 zlay_box_layer(vec2 local, vec2 half_size, float radius, vec4 fill, vec4 border, float border_width, float exponent) {
